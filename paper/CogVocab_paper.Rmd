...
n_CognateList_full_clean <- CognateList_full_clean %>%
  dplyr::count(cognate_status)
```
From the list of 611 translation equivalents, we further excluded any translation equivalent pairs that had complex relationships rather than one-to-one mappings. For example, "noodle" forms a translation equivalent pair with either the French word _"nouilles"_ or _"pâtes"_. These pairs were removed because we could not know which form (e.g., _"nouilles"_ or _"pâtes"_) the infant produced, and we were not able to classify these pairs as either cognates or non-cognates. This left a complete list of `r nrow(CognateList_full_clean)` translation equivalents (`r nrow(CognateList_full_clean[which(CognateList_full_clean$cognate_status=="cognate"),])` cognates and `r nrow(CognateList_full_clean[which(CognateList_full_clean$cognate_status=="non-cognate"),])` non-cognates). 
...
percentage_word ~ age * cognate_status + (1|participant)
...
full_mean_cognates_percentage <- keepers_cognate_full_notPair %>%
  group_by(cognate_status) %>%
  summarize(mean_percent_word = mean(percent_word_acquired, na.rm=T),
            sd_percent_word = sd(percent_word_acquired, na.rm=T),
            min_percent_word = min(percent_word_acquired, na.rm=T),
            max_percent_word = max(percent_word_acquired, na.rm=T))
...
full_lme_model <- keepers_cognate_full_notPair %>%
  ...
  # the lme model
  lmer(percent_word_acquired ~ cognate_status * age_days + (1|subject_id),
                    data = .)

full_lme_model_coefs <- summary(full_lme_model)$coef %>%
  as.data.frame %>%
...
rownames(full_lme_model_coefs) <- c("Intercept", "cognate_status", "age_days", 
                     "cognate_status * age_days") 

# youngest age
youngest <- keepers_cognate_full_notPair %>%
  # keep only the oldest data
  filter(age_days == min(age_days)) %>%
  distinct(age_days)

# oldest age: how many more cognates than non-cognates?
oldest <- keepers_cognate_full_notPair %>%
  # keep only the oldest data
  filter(age_days == max(age_days)) %>%
  distinct(age_days)

oldest_difference <- keepers_cognate_full_notPair %>%
  # keep only the oldest data
  filter(age_days == max(age_days)) %>%
  # calculate the difference between percentage of cognates and percentage of non-cognates
  mutate(difference = percent_word_acquired - lead(percent_word_acquired)) %>%
  # extract the value
  distinct(difference) %>% 
  unname
...
```
Out of the complete list which contained 262 cognate words (i.e., adding the 131 English cognate words and 131 French cognate words) and 812 non-cognate words (i.e., adding the 406 English non-cognate words and 406 French non-cognate words), bilingual infants produced an average of `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="cognate"),2],0)` cognate words (SD = `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="cognate"),3],2)`, range = `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="cognate"),4],0)` – `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="cognate"),5],0)`) and `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="non-cognate"),2],0)` non-cognate words (SD = `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="non-cognate"),3],2)`, range = `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="non-cognate"),4],0)` – `r round(full_mean_cognates[which(full_mean_cognates$cognate_status=="non-cognate"),5],0)`). The percentage of cognate words produced was `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="cognate"),2],2)`% (SD = `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="cognate"),3],2)`, range = `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="cognate"),4],2)` – `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="cognate"),5],2)`), whereas the percentage of non-cognate words produced was `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="non-cognate"),2],2)`% (SD = `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="non-cognate"),3],2)`, range = `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="non-cognate"),4],2)` – `r round(full_mean_cognates_percentage[which(full_mean_cognates_percentage$cognate_status=="non-cognate"),5],2)`). Table 1 shows the coefficient estimates for the model and Figure 1 Panel A visualizes the model. The main effect of cognate status was also significant, suggesting that bilingual infants produced `r round(full_lme_model_coefs[2,1],0)`% more cognate words than non-cognate words at the mean age of `r round(mean_age,1)` days (approximately `r round(mean_age/30,0)` months). The significant effect of age suggested that for every day of increased age, older infants produced `r round(full_lme_model_coefs[3,1],1)`% more non-cognate words (the reference level) than younger infants. The significant interaction between age and cognate status further suggested that the pattern of infants producing more cognate words strengthened as they aged, indicating that for every additional day infants produced `r round(full_lme_model_coefs[4,1],2)`% more cognate words than non-cognate words. As an example, at the youngest age in our sample (~`r round(youngest/30,0)` months or `r youngest` days), the model predicts little to no difference in cognate vs. non-cognate production, versus at the oldest age in our sample (~`r round(oldest/30, 0)` months or `r oldest` days), the model predicts that infants will produce `r round(oldest_difference[1,1],2)`% more cognates than non-cognates.
...
matched_mean_cognates_percentage <- keepers_cognate_matched_notPair %>%
  group_by(cognate_status) %>%
  summarize(mean_percent_word = mean(percent_word_acquired, na.rm=T),
            sd_percent_word = sd(percent_word_acquired, na.rm=T),
            min_percent_word = min(percent_word_acquired, na.rm=T),
...
# linear mixed-effects model
matched_lme_model <- keepers_cognate_matched_notPair %>%
  # scale and center age for ease of interpretation
  mutate(age_days = scale(age_days, scale = FALSE, center = TRUE)) %>%
  # reverse code "cognate_status"
  mutate(cognate_status = fct_rev(cognate_status)) %>%
...
matched_lme_model_coefs <- summary(matched_lme_model)$coef %>%
  as.data.frame %>%
  mutate_at(c("Estimate","Std. Error", "t value", "Pr(>|t|)"), 
            function (x) signif(x, digits = 3)) %>% 
  rename(SE = `Std. Error`, 
...
rownames(matched_lme_model_coefs) <- c("Intercept", "cognate_status", "age_days", 
                     "cognate_status * age_days") 
...
Out of the 162 cognate (i.e., adding the 81 English cognate words and 81 French cognate words) and 162 non-cognate words (i.e., adding the 81 English non-cognate words and 81 French non-cognate words) on the matched list, bilingual infants produced an average of `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="cognate"),2],0)` cognate words (SD = `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="cognate"),3],2)`, range = `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="cognate"),4],0)` – `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="cognate"),5],0)`) and `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="non-cognate"),2],0)` non-cognate words (SD = `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="non-cognate"),3],2)`, range = `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="non-cognate"),4],0)` – `r round(matched_mean_cognates[which(matched_mean_cognates$cognate_status=="non-cognate"),5],0)`). The overall mean percentage of cognate words produced was `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="cognate"),2],1)`% of words (SD = `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="cognate"),3],2)`, range = `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="cognate"),4],2)` – `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="cognate"),5],2)`), whereas the percentage of non-cognate words produced was `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="non-cognate"),2],1)`% (SD = `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="non-cognate"),3],2)`, range = `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="non-cognate"),4],2)` – `r round(matched_mean_cognates_percentage[which(matched_mean_cognates_percentage$cognate_status=="non-cognate"),5],2)`). Table 1 also shows the coefficient estimates for the matched list model and Figure 1 Panel B visualizes the model. Similar to the patterns reported in the complete list model, there were significant effects of age and cognate status. The effect of cognate status suggested that in general bilingual infants produced `r round(matched_lme_model_coefs[2,1],0)`% more cognate words than non-cognate words at the mean age of our sample (`r round(mean_age,1)` days). The age effect suggested that older infants produced `r round(matched_lme_model_coefs[3,1],1)`% more non-cognate words (the reference level) than younger infants. The interaction between cognate and age was also significant: for every additional day infants produced `r round(matched_lme_model_coefs[4,1],2)`% more cognate words than non-cognate words on the matched list. Therefore, overall the direction of effects was similar to the previous analysis (i.e., more cognate words than non-cognate words). 

&nbsp;

```{r Fig1, fig.cap="Percentage of words produced by age and cognate status, with Panel A representing the complete list and Panel B representing the matched list.", echo=FALSE, dpi=600, fig.align='center', fig.height=4, out.width="120%", cache = TRUE}

percent_word_full <- keepers_cognate_full_notPair %>%
  select(subject_id, n_months, age_days, cognate_status, percent_word_acquired) %>%
  rename(full_percent_word = percent_word_acquired)
  
percent_word_matched <- keepers_cognate_matched_notPair %>%
  select(subject_id, n_months, age_days, cognate_status, percent_word_acquired) %>%
  rename(matched_percent_word = percent_word_acquired)

Figure1 <- right_join(percent_word_full, percent_word_matched) %>%
  pivot_longer(c(full_percent_word, matched_percent_word), names_to = "list", values_to = "percent_word") %>%
  mutate(list = recode(list, 
                        full_percent_word = "Panel A: Complete list",
                        matched_percent_word = "Panel B: Matched list"),
         cognate_status = recode(cognate_status,
                                  "cognate" = "Cognate",
                                  "non-cognate" = "Non-cognate")) %>%
  group_by(subject_id, n_months, list, cognate_status) %>%
  ggplot(aes(x = age_days, y = percent_word, color = cognate_status)) +
  facet_wrap(. ~ list) +
  geom_smooth(method = "lm", se = FALSE) + 
  geom_point(stat = "identity", position = "identity", shape = 23, alpha = 0.5) +
  geom_vline(xintercept = 547.6, linetype="dashed", alpha = 0.5) +
  labs(x = "\n Age (in days)", 
       y = "Percentage of words produced (%)",
...
```{r Fig1, fig.cap="Proportion of words produced by age and cognate status, with Panel A representing the complete list and Panel B representing the matched list.", echo=FALSE, dpi=600, fig.align='center', fig.height=4, out.width="120%", cache = TRUE}
...
Table1 <- cbind(full_lme_model_coefs, matched_lme_model_coefs)

papaja::apa_table(Table1, format.args = list(digits = 2),
                  col.names =c("","Estimate","$df$", "$SE$","$t$","$p$","Estimate","$df$", "$SE$","$t$","$p$"),
                  align=c("l","c","c","c","c","c","c","c","c","c"),
                  col_spanners = list("Complete list" = c(2,6), "Matched list" = c(7,11)),
                  caption="Table 1. Coefficient estimates from the linear mixed-effects models predicting percentage of words produced.",
                  landscape = TRUE)
```
...
percentage_pair ~ age * cognate_status + (1|participant)
...
## percentage
full_mean_percent_TE <- keepers_cognate_full_wide %>%
  group_by(cognate_status) %>%
  summarize(mean_TE = mean(percent_TE_acquired, na.rm=T),
            sd_TE = sd(percent_TE_acquired, na.rm=T),
            min_TE = min(percent_TE_acquired, na.rm=T),
            max_TE = max(percent_TE_acquired, na.rm=T))
...
# lme: TE ~ cognate_status * age + (age|subject_id) + (cognate_status|subject_id)
full_lme_final <- keepers_cognate_full_wide %>%
  # reverse-order cognate status, so that the non-cognate status would be the reference level
  mutate(cognate_status = fct_rev(cognate_status)) %>%
  # scale and center age for ease of interpretation
  mutate(age_days = scale(age_days, scale = FALSE, center = TRUE)) %>%
  # the lme model
  lmer(percent_TE_acquired ~ cognate_status * age_days + (1|subject_id),
                    data = .)


full_lme_final_coefs <- summary(full_lme_final)$coef %>%
  as.data.frame %>%
  mutate_at(c("Estimate","Std. Error", "t value", "Pr(>|t|)"), 
            function (x) signif(x, digits = 3)) %>% 
  rename(SE = `Std. Error`, 
...
rownames(full_lme_final_coefs) <- c("Intercept", "cognate_status", "age_days", 
                     "cognate_status * age_days") 
...
Out of the complete list which contained `r nrow(CognateList_full_clean)` translation equivalent pairs (`r nrow(CognateList_full_clean[which(CognateList_full_clean$cognate_status=="cognate"),])` cognates and `r nrow(CognateList_full_clean[which(CognateList_full_clean$cognate_status=="non-cognate"),])` non-cognates), infants produced an average of `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="cognate"),2],0)` cognate pairs (SD = `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="cognate"),3],2)`, range = `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="cognate"),4],0)` – `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="cognate"),5],0)`) and `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="non-cognate"),2],0)` non-cognate pairs (SD = `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="non-cognate"),3],2)`, range = `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="non-cognate"),4],0)` – `r round(full_mean_total_TE[which(full_mean_total_TE$cognate_status=="non-cognate"),5],0)`). The percentage of cognate pairs produced was `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="cognate"),2],1)`% (SD = `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="cognate"),3],2)`, range = `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="cognate"),4],2)` – `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="cognate"),5],2)`) whereas the percentage of non-cognate pairs produced was `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="non-cognate"),2],1)`% (SD = `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="non-cognate"),3],2)`, range = `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="non-cognate"),4],2)` – `r round(full_mean_percent_TE[which(full_mean_percent_TE$cognate_status=="non-cognate"),5],2)`). Table 2 shows the coefficient estimates for the model and Figure 2 Panel A visualizes the model. The significant effect of age suggested that bilingual infants produced `r round(full_lme_final_coefs[3,1],2)`% more non-cognate pairs (the reference level) with every increase in day of age. There was also a significant effect of cognate status, suggesting that on average infants produced `r round(full_lme_final_coefs[2,1],0)`% more translation equivalent pairs that are cognates than those that are non-cognates at the reference age level of `r round(mean_age,1)` days. Likewise, the significant interaction between age and cognate status indicated that the pattern of producing more cognate pairs strengthened as bilingual infants aged. Therefore, bilingual infants produced an even greater percentage of cognate pairs than non-cognate pairs with age. 
...
## percentage
matched_mean_percent_TE <- keepers_cognate_matched_wide %>%
  group_by(cognate_status) %>%
  summarize(mean_TE = mean(percent_TE_acquired, na.rm=T),
            sd_TE = sd(percent_TE_acquired, na.rm=T),
            min_TE = min(percent_TE_acquired, na.rm=T),
...
# lme: TE ~ cognate_status * age + (age|subject_id) + (cognate_status|subject_id)
matched_lme_final <- keepers_cognate_matched_wide %>%
  # reverse-order cognate status, so that the non-cognate status would be the reference level
  mutate(cognate_status = fct_rev(cognate_status)) %>%
  # scale and center age for ease of interpretation
  mutate(age_days = scale(age_days, scale = FALSE, center = TRUE)) %>%
  # the lme model: keep the simplest random effect
  lmer(percent_TE_acquired ~ cognate_status * age_days + (1|subject_id),
                    data = .)

matched_lme_final_coefs <- summary(matched_lme_final)$coef %>%
  as.data.frame %>%
  mutate_at(c("Estimate","Std. Error", "t value", "Pr(>|t|)"), 
            function (x) signif(x, digits = 3)) %>% 
  rename(SE = `Std. Error`, 
...
rownames(matched_lme_final_coefs) <- c("Intercept", "cognate_status", "age_days", 
                     "cognate_status * age_days") 
...
Out of the `r nrow(CognateList_matching)` translation equivalent pairs, bilingual infants produced an average of `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="cognate",2],0)` cognate pairs (SD = `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="cognate",3],2)`, range = `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="cognate",4],0)` – `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="cognate",5],0)`) and `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="non-cognate",2],0)` non-cognate pairs (SD = `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="non-cognate",3],2)`, range = `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="non-cognate",4],0)` – `r round(matched_mean_total_TE[matched_mean_total_TE$cognate_status=="non-cognate",5],0)`). The percentage of cognate pairs produced was `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="cognate"),2],1)`% (SD = `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="cognate"),3],2)`, range = `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="cognate"),4],2)` – `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="cognate"),5],2)`) and the percentage of non-cognate pairs produced was `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="non-cognate"),2],1)`%  (SD = `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="non-cognate"),3],2)`, range = `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="non-cognate"),4],2)` – `r round(matched_mean_percent_TE[which(matched_mean_percent_TE$cognate_status=="non-cognate"),5],2)`). The coefficient estimates for the matched list model is shown in Table 2, and Figure 2 Panel B visualizes the model. Similar to the results for the complete list, the main effects of age and cognate status were significant, as well as the interaction between age and cognate status. The age effect suggested that bilingual infants produced `r round(matched_lme_final_coefs[3,1],2)`% more non-cognate translation equivalent pairs with every day of age. The significant effect of cognate status suggested that overall bilingual infants produced `r round(matched_lme_final_coefs[2,1],0)`% more cognate translation equivalent pairs than non-cognate pairs at the mean age of our sample (`r round(mean_age,1)` days). This pattern strengthened with age as suggested by the significant interaction between age and cognate status, such that bilingual infants produced a greater percentage of cognate pairs than non-cognate pairs as they aged. 

&nbsp;

```{r Fig2, fig.cap="Percentage of translation equivalent pairs produced by age and cognate status, with Panel A representing the complete list and Panel B representing the matched list.", echo=FALSE, dpi=600, fig.align='center', fig.height=4, out.width="120%", cache = TRUE}

percent_TE_full <- keepers_cognate_full_wide %>%
  select(subject_id, n_months, age_days, cognate_status, percent_TE_acquired) %>%
  rename(full_percent_TE = percent_TE_acquired)
  
percent_TE_matched <- keepers_cognate_matched_wide %>%
  select(subject_id, n_months, age_days, cognate_status, percent_TE_acquired) %>%
  rename(matched_percent_TE = percent_TE_acquired)

Figure2 <- right_join(percent_TE_full, percent_TE_matched) %>%
  pivot_longer(c(full_percent_TE, matched_percent_TE), names_to = "list", values_to = "percent_TE") %>%
  mutate(list = recode(list, 
                        full_percent_TE = "Panel A: Full list",
                        matched_percent_TE = "Panel B: Matched list"),
         cognate_status = recode(cognate_status,
                                  "cognate" = "Cognate",
                                  "non-cognate" = "Non-cognate")) %>%
  group_by(subject_id, n_months, list, cognate_status) %>%
  ggplot(aes(x = age_days, y = percent_TE, color = cognate_status)) +
  facet_wrap(. ~ list) +
  geom_smooth(method = "lm", se = FALSE) + 
  geom_point(stat = "identity", position = "identity", shape = 23, alpha = 0.5) +
  geom_vline(xintercept = 547.6, linetype = "dashed", alpha = 0.5) +
  labs(x = "\n Age (in days)", 
       y = "Percentage of translation equivalent pairs produced (%)",
       color = "Cognate status") + 
  theme_minimal() +
  theme(text = element_text(size=12),
        strip.text.x = element_text(size = 12),
...
```{r Fig2, fig.cap="Proportion of translation equivalent pairs produced by age and cognate status, with Panel A representing the complete list and Panel B representing the matched list.", echo=FALSE, dpi=600, fig.align='center', fig.height=4, out.width="120%", cache = TRUE}
...
Table2 <- cbind(full_lme_final_coefs, matched_lme_final_coefs)

papaja::apa_table(Table2, format.args = list(digits = 2),
                  col.names =c("","Estimate","$df$", "$SE$","$t$","$p$","Estimate","$df$", "$SE$","$t$","$p$"),
                  align=c("l","c","c","c","c","c","c","c","c","c"),
                  col_spanners = list("Complete list" = c(2,6), "Matched list" = c(7,11)),
                  caption="Table 2. Coefficient estimates from the linear mixed-effects models predicting percentage of translation equivalent pairs produced.",
                  landscape = TRUE)
```
